...
import fr.ms.log4jdbc.proxy.handler.TimeInvocationOperationFactory;
...
public final class Log4JdbcProxyNew {
    ...
    private static final Log4JdbcOperationFactory createOperationFactory(final Log4JdbcOperationFactory factory) {
	if (devMode) {
	    final TimeInvocationOperationFactory decorator = new TimeInvocationOperationFactory(factory);
	    return decorator;
	} else {
